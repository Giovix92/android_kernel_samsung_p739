...
int p5_check_hwrev(void)
{
	static unsigned int value = 0;

	/* GPIO_HW_REVx shares with DPRAM pin. So use just once */
	...
	if (value == 0)	{
	  ...
	  value = gpio_get_value(GPIO_HW_REV1) |
			  (gpio_get_value(GPIO_HW_REV2)<<1) |
			  (gpio_get_value(GPIO_HW_REV3)<<2) |
			  (gpio_get_value(GPIO_HW_REV4)<<3);
	...
	pr_info("%s: (gpio value = 0x%02x)\n", __func__, value);

	return value; //rev 0.3 : 5(0101)
}
EXPORT_SYMBOL(p5_check_hwrev);
#endif
#endif
