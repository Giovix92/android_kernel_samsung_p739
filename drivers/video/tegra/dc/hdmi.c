...
extern int p5_check_hwrev(void);
#endif
#endif
...
bool tegra_dc_hdmi_hpd(struct tegra_dc *dc, char* call_func_name)
{
	...
	if( flag == HDMI_FLAG_DISABLE ) {
		...
		printk(KERN_INFO	"[HDMI] %s()->%s() hdmi_enable_flag is %d()\n", call_func_name, __func__, flag);
		level = 0;
	}
#else
	level = gpio_get_value(dc->out->hotplug_gpio);
...
//			printk(KERN_INFO	"[HDMI] %s() hdmi_enable_flag is HDMI_FLAG_DISABLE\n", __func__);
//			goto fail;
//		}
//	}
#endif
...
static void tegra_dc_hdmi_resume(struct tegra_dc *dc)
{
	...
	} else if (tegra_dc_hdmi_hpd(dc, __func__)) { /* Check for HDMI Peripheral */
		queue_delayed_work(system_nrt_wq, &hdmi->work,
					   msecs_to_jiffies(100));
	}
	spin_unlock_irqrestore(&hdmi->suspend_lock, flags);
