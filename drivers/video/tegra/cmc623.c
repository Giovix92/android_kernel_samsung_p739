...
#endif
#if !defined(CONFIG_TARGET_LOCALE_KOR)	
    if((cmc623_state.resuming == 1) || (cmc623_state.suspending == 1)) {
        printk("[CMC623:INFO] : skip resume resuming : %d, suspendding : %d\n",cmc623_state.resuming, cmc623_state.suspending);    
        return; 
    }
...
#elif (CONFIG_MACH_SAMSUNG_P5_CTC)  
	cmc623_state.gpio.bl_reset = GPIO_BL_RESET;
	cmc623_state.gpio.mlcd_on = GPIO_MLCD_ON;
	if (system_rev < 6)
		cmc623_state.gpio.mlcd_on = TEGRA_GPIO_PI4;
	if (system_rev < 6)
		cmc623_state.gpio.bl_reset = TEGRA_GPIO_PB0;
