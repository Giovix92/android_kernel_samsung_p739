...
static void utmi_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
	unsigned long val;
	void __iomem *base = phy->regs;
...
int tegra_usb_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
...
#ifdef CONFIG_MACH_SAMSUNG_VARIATION_TEGRA
	if (usb_phy_data[phy->instance].usb_ldo_en && is_dpd)
	...
	if (phy->instance == 1) {
		...
		utmi_phy_power_on(phy, is_dpd);
	}
	return 0;
}

int tegra_usb_phy_power_off(struct tegra_usb_phy *phy, bool is_dpd)
{
	if (phy->instance == 1) {
		struct tegra_ulpi_config *ulpi_config = phy->config;
		if (ulpi_config->inf_type == TEGRA_USB_LINK_ULPI)
...
#if defined(CONFIG_MACH_SAMSUNG_VARIATION_TEGRA) && !defined(CONFIG_MACH_SAMSUNG_P4LTE)
	if (phy->regulator_on && is_dpd) {
		if (phy->instance == 1) {
			gpio_set_value(GPIO_HSIC_EN, 0);
			pr_debug("SMD PHY HSIC_EN -> 0\n");
		}
#else
	if (phy->regulator_on && (tegra_get_revision() >= TEGRA_REVISION_A03) && is_dpd) {
#endif
#ifndef CONFIG_MACH_SAMSUNG_VARIATION_TEGRA
