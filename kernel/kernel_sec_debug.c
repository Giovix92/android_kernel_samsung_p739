...
#if 1 // defined(CONFIG_DPRAM_CDMA)
volatile unsigned int *g_idpram_base=NULL;
...
void kernel_sec_cdma_dpram_dump(void)
{
	...
	memcpy(g_cdma_dpram_buf.dpram_buf, g_idpram_base, DPRAM_BUF_SIZE);
	printk(KERN_EMERG "buf address (0x%x), dpram (0x%x)\n", g_cdma_dpram_buf.dpram_buf, g_idpram_base );

	for (index = 0; index < 4; index++)	{
		printk(KERN_EMERG "buf data: %02x %02x %02x %02x\n",
				g_cdma_dpram_buf.dpram_buf[0 + (4 * index)],
				g_cdma_dpram_buf.dpram_buf[1 + (4 * index)],
				g_cdma_dpram_buf.dpram_buf[2 + (4 * index)],
				g_cdma_dpram_buf.dpram_buf[3 + (4 * index)]);
	}
}
...
#define IDPRAM_PHYSICAL_ADDR 			0xD0000000
#else
...
#define IDPRAM_SIZE 					0x4000
	...
	*/
	...
	/* This is for DPRAM (CDMA) */
	g_idpram_base = (volatile void *)ioremap_nocache(IDPRAM_PHYSICAL_ADDR, IDPRAM_SIZE); 
	if(g_idpram_base == NULL) {
