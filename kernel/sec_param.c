...
bool sec_open_param_forced(void)
{
	int ret = true;
	int offset = SEC_MISC_FILE_OFFSET;
...
EXPORT_SYMBOL(sec_open_param_forced);
...
bool sec_get_param(sec_param_index index, void *value)
{
	...
	case param_index_sbk:
		memcpy(value, &(param_data->sbk), sizeof(unsigned int) * 4);
		break;
#ifdef param_test
...
bool sec_set_param(sec_param_index index, void *value)
{
	...
	case param_index_fusetrigger:
		...
		break;
	case param_index_sbk:
		memcpy(&(param_data->sbk), value, sizeof(unsigned int) * 4);
		break;
#ifdef param_test
	case param_index_test:
